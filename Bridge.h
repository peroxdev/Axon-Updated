...
namespace Bridge
{
	...
	void push(lua_State* L, DWORD rL, int index)
	{
		...
		case LUA_TFUNCTION:
			...
			printf("pushnumber passed\n");
			rlua_pushcclosure(rL, Bridge::int3breakpoints[0], 1);
			printf("pushcclosure pass\n");
			break;
		case LUA_TTABLE:
			lua_pushvalue(L, index);
			r_lua_newtable(rL);
	...
	void push(DWORD rL, lua_State* L, int index)
	{
		...
		case R_LUA_TTABLE:
			...
			lua_newtable(L);
			rlua_pushnil(rL);
			while (rlua_next(rL, -2) != R_LUA_TNIL)
			{
				Bridge::push(rL, L, -2);
				Bridge::push(rL, L, -1);
				lua_settable(L, -3);
	...
	int vanillaFunctionBridge(lua_State* L)
	{
		rlua_pushstring(m_rL, std::to_string(++registry).c_str());
		DWORD rL = rlua_newthread(m_rL);
		rlua_settable(m_rL, LUA_REGISTRYINDEX);
		...
		if (rlua_pcall(rL, lua_gettop(L), LUA_MULTRET, 0))
		{
		...
		for (int arg = 1; arg <= rlua_gettop(rL); ++arg, ++args)
	...
	int rbxFunctionBridge(DWORD rL)
	{
		...
		int key = rlua_tonumber(rL, lua_upvalueindex(1));
		...
		switch (lua_pcall(L, rlua_gettop(rL), LUA_MULTRET, 0))
		{
